...
public class FileService : IFileService
{
	...
	{
		return TransferInternal(photos, outputFolder, isDryRun, true, false);
	}

	public IReadOnlyCollection<Photo> Move(IReadOnlyCollection<Photo> photos, string outputFolder, bool isDryRun)
	{
		return TransferInternal(photos, outputFolder, isDryRun, true, true);
	}

	public IReadOnlyCollection<Photo> CopyIfNotExists(IReadOnlyCollection<Photo> photos, string outputFolder, bool isDryRun)
	{
		return TransferInternal(photos, outputFolder, isDryRun, false, false);
	}

	private IReadOnlyCollection<Photo> TransferInternal(IReadOnlyCollection<Photo> photos, string outputFolder, bool isDryRun, bool breakOnDestinationExist, bool move)
	{
		var photosTransferred = new List<Photo>();
		foreach (var photo in photos)
		{
			...
			if (photo.PhotoFile.TargetFullPath == null)
				throw new PhotoCliException($"Couldn't {(move ? "move" : "copy")} main photo file {photo.PhotoFile.SourceFullPath} without a {nameof(PhotoFile.TargetFullPath)}");
			...
			var actionVerb = move ? "moving" : "copying";
			var actionPast = move ? "moved" : "copied";

			_logger.LogTrace("Photo is {Action} to: {To} from: {From}", actionVerb, photo.PhotoFile.TargetFullPath, photo.PhotoFile.SourcePath);
			try
			{
				...
				{
					if (move)
						fileInfo.MoveTo(photo.PhotoFile.TargetFullPath);
					else
						fileInfo.CopyTo(photo.PhotoFile.TargetFullPath, false);
				}

				if (move)
					++_statistics.PhotosMoved;
				else
				...
				if (photo.CompanionFiles != null)
				{
					foreach (var companionFile in photo.CompanionFiles)
					{
						...
						if (companionFile.TargetFullPath == null)
							throw new PhotoCliException($"Couldn't {actionVerb} companion file {companionFile.SourceFullPath} without a {nameof(PhotoFile.TargetFullPath)}");
						...
						{
							if (move)
								companionFileInfo.MoveTo(companionFile.TargetFullPath);
							else
								companionFileInfo.CopyTo(companionFile.TargetFullPath, false);
						}

						if (move)
							++_statistics.CompanionFilesMoved;
						else
							++_statistics.CompanionFilesCopied;
					}
				}

				photosTransferred.Add(photo);
			}
			catch (IOException ioException)
			{
				_statistics.FileIoErrors.Add(ioException.Message);
				_logger.LogCritical(ioException, "Can't {Action} file", actionVerb);
			}
			_logger.LogInformation("Photo is {Action} to: {To} from: {From}", actionPast, photo.PhotoFile.TargetFullPath, fileInfo.FullName);
		}

		return photosTransferred;
	}
