...
public static class Program
{
	...
	public static Task<int> MainStream(string[] args, TextWriter textWriter)
	{
		...
		switch (baseOptions)
		{
			...
			case IngestOptions ingestOptions:
			{
				var validationResultIngest = new IngestOptionsValidator().Validate(ingestOptions);
				if (!validationResultIngest.IsValid)
				{
					WriteErrorOutputValidationErrors(validationResultIngest, textWriter);
					return ReturnExitCode(ExitCode.IngestOptionsValidationFailed);
				}

				host = BuildHostWithReverseGeocode<IngestRunner, IngestOptions>(ingestOptions, textWriter);
				break;
			}
			default:
	...
	public static async Task<int> MainWithServiceProvider(IServiceProvider serviceProvider)
	{
		...
		{
			System.Console.Error.WriteLine($"{Constants.AppSettingsFileName} has some invalid settings. Undo or reset all settings via `{OptionNames.ApplicationAlias} {OptionNames.SettingsVerb} --{OptionNames.ResetOptionNameLong}`");
			foreach (var validationResultError in toolOptionsValidationResult.Errors)
				System.Console.Error.WriteLine(validationResultError);
	...
	private static IHost BuildHostCore<TConsoleRunner>(TextWriter textWriter, Action<IServiceCollection, IConfigurationRoot>? additionalConfigureServices) where TConsoleRunner : IConsoleRunner
	{
		...
		var builder = new HostBuilder().ConfigureServices(services =>
		{
			...
			services.AddTransient(typeof(TConsoleRunner));
